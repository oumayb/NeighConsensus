...
class Conv4d(_ConvNd):
    ...
    def forward(self, input):
        return conv4d(input, self.weight, bias=self.bias,permute_filters=not self.pre_permuted_filters) # filters pre-permuted in constructor

if __name__ == '__main__' : 
    totoConv4D = Conv4d(1, 16, 3)
    a = torch.randn(1, 1, 11, 12, 13 ,14).cuda()
    deviceID = a.get_device()
    totoConv4D.cuda(deviceID)
    print (totoConv4D(a).size()) # output is 1, 16, 11, 12, 13, 14
